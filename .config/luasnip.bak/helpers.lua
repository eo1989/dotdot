...
local snip_defs = {
  ...
  -- override to enable restore cursor
...
local function setup_snip_env() setfenv(2, vim.tbl_extend('force', _G, snip_defs)) end

local function edit_fts()
  local fts = require('luasnip.util.util').get_snippet_filetypes()
  vim.ui.select(fts, {
    prompt = 'Select which filetype to edit:',
  }, function(item, idx)
    if idx then vim.cmd('edit /Users/eo/.config/nvim/luasnippets/' .. item .. '.lua') end
  end)
end
