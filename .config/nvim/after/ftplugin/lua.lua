  ...
  surround.buffer_setup({
    surrounds = {
      ...
      F = {
        add = function()
          return {
            { string.format('local function %s() ', get_input('Enter a function name')) },
            { ' end' },
          }
        end,
      },
      i = {
        add = function()
          return {
            { string.format('if %s then ', get_input('Enter a condition')) },
            { ' end' },
          }
        end,
      },
      t = {
        add = function()
          return {
            { string.format('{ %s = { ', get_input('Enter a field name')) },
            { ' }}' },
          }
        end,
      },
...
-- vim.keymap.set('n', 'K', kw, { buffer = 0 })
