...
local function get_black_length(bufnr)
  local query = vim.treesitter.query.parse(
    ...
    [[
      (table
        (dotted_key) @dotkey (#eq? @dotkey "tool.black" || #eq? @dotkey "tool.ruff")
        (pair
  ...
  for id, node in query:iter_captures(root, bufnr, 0, -1) do
    ...
    if name == 'length' then return vim.treesitter.get_node_text(node, bufnr) end
  end
end
...
vim.api.nvim_create_autocmd('LspAttach', {
  callback = function(args)
    ...
    if vim.bo[buffer].ft ~= 'python' then return end
    ...
    vim.opt_local.colorcolumn = tostring(length or '80')
