local function get_std_envs()
  local envs = {}
  if vim.env.VIRTUAL_ENV then envs['VIRTUAL_ENV'] = table.concat({ vim.env.VIRTUAL_ENV, 'bin', 'python' }, '/') end
...
local function get_poetry_envs()
  ...
  for match in string.match(venv, '[%w%/%.%-]*virtualenv[%w%/%.%-]*') do
...
local function get_python_client()
  ...
  local bpyright = vim.lsp.get_clients { name = 'basedpyright' }
  if #clients > 0 then return clients[1] end
end

local function env_handler()
  local client = get_python_client()
  if not client then vim.notify('No python clients found') end
  local options = vim.tbl_contains('force', get_std_envs(), get_poetry_envs())
  local settings = client.config.settings
  local active = settings.python.pythonPath
  vim.ui.select(vim.tbl_keys(options), {
    prompt = 'Select a Python Environment:',
    format_item = function(item)
      if options[item] == active then return item .. ' (Active)' end
      return item
    end,
  }, function(selected)
    if selected then
