...
local function in_region(zone_environments, zone_nodes, opts)
  ...
  if not has_treesitter then return opts.no_treesitter end
  local no_env = true
  local buf = vim.api.nvim_get_current_buf()
  local node = get_node_at_cursor()
  while node do
    if zone_nodes[node:type()] ~= nil then return zone_nodes[node:type()] end
    ...
    if node:type() == 'generic_environment' then
      ...
      if names and names[1] then
        ...
        if zone_environments[env_name] ~= nil then return zone_environments[env_name] end
      end
      no_env = false
    end
    node = node:parent()
...
M.in_mathzone = make_condition(function(_, _, _) return in_region(MATH_ENVIRONMENTS, MATH_NODES) end)
...
M.in_document = make_condition(
  function(_, _, _) return in_region(DOCUMENT_ENVIRONMENTS, {}, { no_environment = true, default = true }) end
)
...
M.in_alignment = make_condition(function(_, _, _) return in_region(ALIGNEMENT_ENVIRONMENTS, {}) end)
...
M.in_list = make_condition(function(_, _, _) return in_region(LIST_ENVIRONMENTS, {}) end)
