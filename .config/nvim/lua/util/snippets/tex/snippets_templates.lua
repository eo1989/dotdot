...
function M.simple_env_nodes(envname, restore)
  ...
  local middle_node = restore and restore == _G.luasnip._cache.simple_env_nodes_restore_key and r(1, restore)
    ...
    or i(1)
  if restore then _G.luasnip._cache.simple_env_nodes_restore_key = restore end
  return fmta(
    [[
  \begin{<>}
     <>
...
function M.named_env_nodes(envname, restore)
  ...
  local middle_node = restore and restore == _G.luasnip._cache.named_env_nodes_restore_key and r(2, restore)
    ...
    or i(2)
  if restore then _G.luasnip._cache.named_env_nodes_restore_key = restore end
  return fmta(
    [[
  \begin{<>}<>
     <>
...
function M.capture_wrap_snippet(context, command, opts)
  ...
  local body = f(function(_, snip) return command .. [[{]] .. snip.captures[1] .. [[}]] end)
...
function M.get_visual(alternative, transform)
  local transform_function = transform or function(visual) return i(1, visual) end
...
function M.simple_command_snippet(context, command, opts, tmpl_opts)
  ...
  body = tmlp_opts.visual
      and d(1, M.get_visual(body, function(selection) return t(command .. [[{]] .. selection .. [[}]]) end))
    or body

  s(context, body, opts)
end

local no_backslash = require('luasnip.extras.conditions').make_condition(
  function(line, trig, _) return string.sub(line, #line - #trig, #line - #trig) ~= '\\' end
)
