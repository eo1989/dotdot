...
local L, M, S = lsp.log_levels, lsp.protocol.Methods, vim.diagnostic.severity

local extensions, border = require('eo.lsp.extensions'), eo.ui.current.border
...
api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function(event)
    ...
    --   --   map('n', 'gd', function() lsp.buf.definition { on_list = create_on_list(client) } end, {
    --   --     buffer = bufnr,
    ...
    _map('gr', 'lsp: [g]oto [r]references', function() lsp.buf.references { includeDeclaration = false } end)

    _map('<leader>rn', 'lsp: [r]e[n]ame all symbol references', function() lsp.buf.rename() end)

    -- _map('x', '<leader>ca', 'lsp: select [c]ode [a]ction', function() lsp.buf.code_action() end)
    map({ 'n', 'x' }, '<leader>ca', function() lsp.buf.code_action() end, { desc = 'lsp: select [c]ode [a]ction' })

    _map('<leader>cl', 'lsp: run [c]ode [l]ens', function() lsp.codelens.run() end)

    _map('<leader>ci', 'lsp: toggle [i]nlay [h]int for buffer', function()
      local is_enabled = lsp.inlay_hint.is_enabled { bufnr = buf }
      lsp.inlay_hint.enable(not is_enabled, { bufnr = buf })
    end)

    _map('<leader>iH', 'lsp: toggle [i]nlay [H]int globally', function()
    ...
    _map('<leader>rN', 'lsp: [r]ename [f]ile', function() rename_file(client) end)
