...
-- since thats loaded on ea run of our qf window
-- this emans that it would be recreated ea time if
-- not defined separately, so replacing the qf
-- we would recreate this func during its execution
-- source: https://vi.stackexchange.com/a/21255
-- using range-aware func
function as.qf.delete(bufnr)
  bufnr = bufnr or api.nvim_get_current_buf()
  local qfl = fn.getqflist()
  local line = unpack(api.nvim_win_get_cursor(0))
  -- FIXME: get visual selection so this functionality can work in vis mode
  ...
  -- replace items in the current list, dont make a new copy of it;
