...
def erfcc(x: float | np.ndarray) -> float | np.ndarray:
    """
...
@njit(cache=False, fastmath=True)
def ncdf(x: float | np.ndarray) -> float | np.ndarray:
    return 1.0 - 0.5 * erfcc(ONE_OV_SQRT_2 * x)


@njit(cache=False, fastmath=True)
def npdf(x: float | np.ndarray) -> float | np.ndarray:
    return ONE_OV_SQRT_2PI * np.exp(-0.5 * np.square(x))


@njit(cache=False, fastmath=True)
def inv_erf(x: float | np.ndarray) -> float | np.ndarray:
    """
...
@njit(cache=False, fastmath=True)
def ncdf_inv(x: float | np.ndarray) -> float | np.ndarray:
    """
...
bsm_price_vector = np.vectorize(
    compute_bsm_vanilla_price, doc="Vectorized `bsm_vanilla_price` function"
)


@njit
def compute_bsm_vanilla_slice_prices(
    ttm: float,
    forward: float,
    strikes: np.ndarray,
    vols: np.ndarray,
    ...
    def f(strike: float, vol: float, optype: str) -> float:
        return compute_bsm_vanilla_price(
            forward=forward,
            strike=strike,
            ttm=ttm,
            vol=vol,
...
@njit
def compute_bsm_forward_grid_prices(
    ttm: float,
    forwards: np.ndarray,
    strike: float,
    vol: float,
...
@njit
def compute_bsm_vanilla_slice_deltas(
    ttm: float,
    forward: float,
    strikes: np.ndarray,
    vols: np.ndarray,
    ...
    def f(strike: float, vol: float, optype: str) -> float:
        return compute_bsm_vanilla_delta(
...
@njit
def compute_bsm_vanilla_deltas_ttms(
    ttms: np.ndarray,
    forwards: np.ndarray,
...
@njit
def compute_bsm_vanilla_grid_deltas(
    ttm: float,
    forwards: np.ndarray,
    strike: float,
    vol: float,
...
def compute_bsm_strike_from_delta(
...
@njit
def compute_bsm_vanilla_slice_vegas(
    ttm: float,
    forward: float,
    strikes: np.ndarray,
    vols: np.ndarray,
...
def compute_bsm_vegas_ttms(
    ...
    for ttm, forward, vols_ttm, strikes_ttm, optypes_ttm in zip(
        ...
        vegas_ttms.append(
            compute_bsm_vanilla_slice_vegas(
                ttm=ttm,
                forward=forward,
                strikes=strikes_ttm,
                vols=vols_ttm,
...
@njit
def compute_bsm_vanilla_vega_ttms(
    ttms: np.ndarray,
    forwards: np.ndarray,
    strikes_ttms: List[np.ndarray],
    vols_ttms: List[np.ndarray],
