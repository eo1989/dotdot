...
eza_binary="${BREW_PREFIX}/bin/eza"
...
function _list_files_here {
    if [[ ! -x "$(command -v '/opt/homebrew/bin/eza')" ]]; then print "\033[0;33mMagic Dashboard: \`eza\` not installed.\033[0m" && return 1; fi
...
function _magic_enter {
    ...
    [[ "$(/opt/homebrew/bin/eza --git-ignore | wc -l)" -gt 0 ]] && echo
    _magic_dashboard
}
